...
public interface PassportController {
    ...
    PassportResponseDto addResidenceToPassport(@PathVariable("id") Integer id, @RequestBody ResidenceDto residenceDto);
    @Operation(summary = "This is endpoint to add registration to passport by ID.", description = "Create request to add registration.", tags = {"Passport"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "SUCCESS. Registration was added."),
            @ApiResponse(responseCode = "400", description = "Invalid input."),
            @ApiResponse(responseCode = "404", description = "NOT FOUND. Passport does not exist."),
            @ApiResponse(responseCode = "500", description = "Server error.")})
    @PatchMapping("/passports/deactivate/res/{country}")
    List<PassportResponseDto> deactivateResidence(String country);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "500", description = "Server error.")})
    @GetMapping("/passports/res/{id}")
    @ResponseStatus(HttpStatus.OK)
    PassportResponseDto findPassportByResidence(@PathVariable("id") UUID id);
}
